...
(function () {
  ...
  const GAMES = {
    snake: {
      ...
      category: "code",
      accent: C.cyan,
      instructions: "Arrow keys / WASD — eat 8 code bytes without crashing.",
      duration: 55000,
      create(w, h, keys) {
        ...
        return {
          update() {
            ...
            if (head.x === food.x && head.y === food.y) {
              eaten++;
              if (eaten >= 8) return endSession("win");
              placeFood();
          ...
          draw(ctx) {
            ...
            drawHud(ctx, w, `BYTES: ${eaten}/8`, C.cyan);
    ...
    space_invaders: {
      ...
      category: "code",
      accent: C.cyan,
      instructions: "← → move, SPACE shoot — clear all alien PRs.",
      duration: 60000,
      create(w, h, keys) {
        let px = w / 2, bullets = [], aliens = [], frame = 0, dir = 1;
        for (let r = 0; r < 3; r++) for (let c = 0; c < 8; c++)
          aliens.push({ x: 40 + c * 38, y: 40 + r * 28, alive: true });
        return {
          update() {
            if (keys.left) px = Math.max(20, px - 5);
            if (keys.right) px = Math.min(w - 20, px + 5);
            if (keys.fire && frame % 12 === 0) bullets.push({ x: px, y: h - 35, vy: -7 });
            frame++;
            ...
            aliens.forEach(a => {
              if (!a.alive) return;
              bullets.forEach(b => {
                if (Math.abs(b.x - a.x) < 16 && Math.abs(b.y - a.y) < 14) { a.alive = false; b.y = -99; }
              });
              if (a.y > h - 60) endSession("lose", "Aliens merged to main!");
            });
    ...
    breakout: {
      ...
      category: "code",
      accent: C.cyan,
      instructions: "← → paddle, SPACE launch — smash all legacy code bricks.",
      duration: 55000,
      create(w, h, keys) {
        let paddle = w / 2, ball = { x: w / 2, y: h - 50, vx: 3, vy: -4, stuck: true };
        let bricks = [];
        for (let r = 0; r < 4; r++) for (let c = 0; c < 10; c++)
          bricks.push({ x: 10 + c * 38, y: 35 + r * 18, w: 34, h: 14, alive: true, color: [C.cyan, C.purple, C.gold, C.red][r] });
        return {
          update() {
            ...
            if (ball.y > h - 40 && Math.abs(ball.x - paddle) < 34) { ball.vy = -Math.abs(ball.vy); ball.vx += (ball.x - paddle) * 0.08; }
            if (ball.y > h) return endSession("lose", "Ball fell to /dev/null!");
            bricks.forEach(b => {
              if (!b.alive) return;
    ...
    asteroids: {
      ...
      category: "code",
      accent: C.cyan,
      instructions: "WASD thrust/rotate, SPACE shoot — destroy 12 heap fragments.",
      duration: 60000,
      create(w, h, keys) {
        ...
        return {
          update() {
            ...
            rocks.forEach(r => {
              bullets.forEach(b => {
                ...
                if (d < r.r) { b.life = 0; r.r = -1; killed++; if (killed >= 12) endSession("win"); else if (rocks.filter(x => x.r > 0).length === 0) spawn(); }
              });
              if (Math.hypot(ship.x - r.x, ship.y - r.y) < r.r + 8) endSession("lose", "Ship garbage collected!");
            });
          ...
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `FRAGMENTS: ${killed}/12`, C.cyan);
    ...
    pong: {
      ...
      category: "design",
      accent: C.purple,
      instructions: "↑ ↓ move paddle — score 5 points vs the latency bot.",
      duration: 50000,
      create(w, h, keys) {
        let py = h / 2, ay = h / 2, ball = { x: w / 2, y: h / 2, vx: 4, vy: 3 }, ps = 0, as = 0;
        return {
          update() {
            if (keys.up) py = Math.max(30, py - 5);
            if (keys.down) py = Math.min(h - 30, py + 5);
            ay += (ball.y - ay) * 0.06;
            ball.x += ball.vx; ball.y += ball.vy;
            ...
            if (ball.x < 0) { as++; ball = { x: w / 2, y: h / 2, vx: 4, vy: randF(-3, 3) }; if (as >= 5) endSession("lose", "Latency bot wins!"); }
            if (ball.x > w) { ps++; ball = { x: w / 2, y: h / 2, vx: -4, vy: randF(-3, 3) }; if (ps >= 5) endSession("win"); }
          },
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
    ...
    memory: {
      ...
      category: "design",
      accent: C.purple,
      instructions: "Click cards — match all 6 pairs of design tokens.",
      duration: 70000,
      create(w, h, keys, mouse) {
        ...
        const cols = 4, rows = 3, cw = (w - 20) / cols, ch = (h - 40) / rows;
        mouse.click = (mx, my) => {
          if (lock) return;
          ...
          if (sel.length === 2) {
            ...
            else setTimeout(() => { sel.forEach(c => c.open = false); sel = []; lock = false; }, 500);
          }
        };
        return {
          update() {},
    ...
    frogger: {
      ...
      category: "design",
      accent: C.purple,
      instructions: "Cross 5 lanes of traffic & scope creep to reach prod.",
      duration: 55000,
      create(w, h, keys) {
        let fx = w / 2, fy = h - 40, lane = 0, frame = 0;
        const lanes = 5, cars = [];
        for (let l = 0; l < lanes; l++) for (let i = 0; i < 3; i++)
          cars.push({ lane: l, x: randF(0, w), spd: (l % 2 ? 1 : -1) * (2 + l * 0.5) });
        return {
          update() {
            frame++;
            if (keys.up && frame % 8 === 0 && fy > 50) { fy -= 22; lane++; }
            if (keys.down && frame % 8 === 0 && fy < h - 40) { fy += 22; lane = Math.max(0, lane - 1); }
            if (keys.left) fx = Math.max(16, fx - 4);
            if (keys.right) fx = Math.min(w - 16, fx + 4);
            cars.forEach(c => { c.x += c.spd; if (c.x < -30) c.x = w + 30; if (c.x > w + 30) c.x = -30; });
            if (lane > 0) cars.forEach(c => {
              const ly = 50 + c.lane * 28;
              if (Math.abs(fy - ly) < 14 && Math.abs(fx - c.x) < 22) endSession("lose", "Run over by a deadline!");
            });
    ...
    tetris: {
      ...
      category: "design",
      accent: C.purple,
      instructions: "← → move, ↓ drop, ↑ rotate — clear 8 lines of tech debt.",
      duration: 65000,
      create(w, h, keys) {
        ...
        return {
          ...
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `LINES: ${lines}/8`, C.purple);
    ...
    flappy: {
      ...
      category: "design",
      accent: C.purple,
      instructions: "SPACE / click flap — pass 10 scope gates.",
      duration: 50000,
      create(w, h, keys, mouse) {
        let bird = { y: h / 2, vy: 0 }, pipes = [], passed = 0, frame = 0;
        mouse.click = () => { bird.vy = -5; };
        return {
          update() {
            frame++;
            if (keys.fire) bird.vy = -5;
            bird.vy += 0.28; bird.y += bird.vy;
            if (frame % 70 === 0) pipes.push({ x: w, gap: randF(80, 140), scored: false });
            pipes.forEach(p => {
              p.x -= 3;
              if (!p.scored && p.x + 30 < 60) { p.scored = true; passed++; if (passed >= 10) endSession("win"); }
              if (60 > p.x && 60 < p.x + 30 && (bird.y < p.gap - 40 || bird.y > p.gap + 40)) endSession("lose", "Hit a scope gate!");
            });
          ...
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `GATES: ${passed}/10`, C.purple);
            pipes.forEach(p => {
              ctx.fillStyle = C.purple;
    ...
    whack_bug: {
      ...
      category: "polish",
      accent: C.gold,
      instructions: "Click the 🐛 bugs! Squash 15 before they escape.",
      duration: 45000,
      create(w, h, keys, mouse) {
        let holes = Array.from({ length: 9 }, (_, i) => ({ i, bug: false, timer: rand(20, 80) }));
        ...
        mouse.click = (mx, my) => {
          ...
          if (hole && hole.bug) { hole.bug = false; squashed++; if (squashed >= 15) endSession("win"); }
        };
        return {
          update() {
            holes.forEach(h => {
              h.timer--;
              if (h.timer <= 0) {
                if (h.bug) { missed++; if (missed >= 5) endSession("lose", "Bugs shipped to prod!"); }
                h.bug = Math.random() < 0.55;
                h.timer = rand(25, 70);
              }
            });
          },
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `SQUASHED: ${squashed}/15  MISSED: ${missed}/5`, C.gold);
    ...
    pac_dots: {
      ...
      create(w, h, keys) {
        ...
        for (let y = 0; y < rows; y++) for (let x = 0; x < cols; x++)
          if ((x + y) % 3 !== 0 && !(x === 1 && y === 1)) dots.push({ x, y });
        return {
          update() {
            let nx = px, ny = py;
            if (keys.left) nx--;
            ...
            ghosts.forEach(g => {
              if (Math.random() < 0.04) { g.x += rand(-1, 1); g.y += rand(-1, 1); g.x = clamp(g.x, 0, cols - 1); g.y = clamp(g.y, 0, rows - 1); }
    ...
    minesweeper: {
      ...
      category: "polish",
      accent: C.gold,
      instructions: "Click to reveal — flag 10 mines without detonating.",
      duration: 70000,
      create(w, h, keys, mouse) {
        ...
        mouse.click = (mx, my) => {
          ...
          if (revealed >= cols * rows - 10) endSession("win");
        };
        mouse.right = (mx, my) => {
          const c = Math.floor((mx - 5) / cw), r = Math.floor((my - 30) / ch);
          if (c < 0 || c >= cols || r < 0 || r >= rows) return;
          if (grid[r][c] === 0) { grid[r][c] = -2; flags++; if (flags >= 10) endSession("win"); }
        };
        return {
          update() {},
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `FLAGS: ${flags}/10  SAFE: ${revealed}`, C.gold);
    ...
    reaction: {
      ...
      category: "polish",
      accent: C.gold,
      instructions: "SPACE when the bar hits green — 8 perfect pings.",
      duration: 40000,
      create(w, h, keys) {
        ...
        return {
          update() {
            ...
            if (keys.fire) {
              if (pos >= 42 && pos <= 58) { hits++; waiting = true; wait = rand(20, 50); pos = 0; if (hits >= 8) endSession("win"); }
          ...
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `PINGS: ${hits}/8`, C.gold);
            if (!waiting) {
              ctx.fillStyle = "#222";
              ctx.fillRect(40, h / 2 - 15, w - 80, 30);
              ctx.fillStyle = "rgba(57,255,20,0.5)";
              ctx.fillRect(40 + (w - 80) * 0.42, h / 2 - 15, (w - 80) * 0.16, 30);
              ctx.fillStyle = C.gold;
    ...
    invaders_rush: {
      ...
      category: "code",
      accent: C.cyan,
      instructions: "SPACE shoot — destroy 20 deploy blobs before they land.",
      duration: 50000,
      create(w, h, keys) {
        ...
        return {
          update() {
            ...
            if (keys.left) px = Math.max(20, px - 5);
            if (keys.right) px = Math.min(w - 20, px + 5);
            if (keys.fire && frame % 8 === 0) bullets.push({ x: px, y: h - 30, vy: -8 });
            if (frame % 35 === 0) blobs.push({ x: randF(20, w - 20), y: 30, vy: randF(1.5, 3) });
            bullets.forEach(b => { b.y += b.vy; });
            bullets = bullets.filter(b => b.y > 20);
            blobs.forEach(b => {
              b.y += b.vy;
              bullets.forEach(bl => { if (Math.abs(bl.x - b.x) < 14 && Math.abs(bl.y - b.y) < 14) { b.y = 999; bl.y = -99; killed++; } });
              if (b.y > h - 40 && Math.abs(b.x - px) < 20) endSession("lose", "Deploy failed on prod!");
              if (b.y > h) endSession("lose", "Blob reached main branch!");
            });
            blobs = blobs.filter(b => b.y < h);
            if (killed >= 20) endSession("win");
          },
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `BLOBS: ${killed}/20`, C.cyan);
            ctx.fillStyle = C.cyan;
    ...
    color_catch: {
      ...
      category: "design",
      accent: C.purple,
      instructions: "← → catch 12 falling hex codes in your palette bucket.",
      duration: 50000,
      create(w, h, keys) {
        ...
        return {
          update() {
            ...
            if (frame % 40 === 0) drops.push({ x: randF(20, w - 20), y: 30, color: colors[rand(0, colors.length - 1)] });
            drops.forEach(d => { d.y += 3; if (d.y > h - 35 && Math.abs(d.x - bx) < 30) { caught++; d.y = 999; if (caught >= 12) endSession("win"); } });
          ...
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `CAUGHT: ${caught}/12`, C.purple);
            drops.forEach(d => { ctx.fillStyle = d.color; ctx.fillRect(d.x - 8, d.y, 16, 16); });
            ctx.fillStyle = C.purple;
            ctx.fillRect(bx - 35, h - 28, 70, 12);
          }
        };
      }
    },
    ...
    stack_jump: {
      ...
      category: "polish",
      accent: C.gold,
      instructions: "SPACE jump — land on 10 stack frames without falling.",
      duration: 45000,
      create(w, h, keys) {
        let player = { x: 80, y: h - 60, vy: 0, onGround: true }, frames = [], score = 0, scroll = 0;
        for (let i = 0; i < 8; i++) frames.push({ x: 60 + i * 55, y: h - 50 - i * 35, w: 50 });
        return {
          update() {
            if (keys.fire && player.onGround) { player.vy = -9; player.onGround = false; }
            player.vy += 0.4; player.y += player.vy;
            scroll += 1.5;
            frames.forEach(f => { f.x -= 1.5; if (f.x < -60) { f.x = w + rand(0, 80); f.y = rand(h - 200, h - 80); } });
            player.onGround = false;
            frames.forEach(f => {
              if (player.vy >= 0 && player.x > f.x && player.x < f.x + f.w && player.y >= f.y - 8 && player.y <= f.y + 8) {
                player.y = f.y - 8; player.vy = 0; player.onGround = true;
                if (!f.landed) { f.landed = true; score++; if (score >= 10) endSession("win"); }
              }
            });
          ...
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `FRAMES: ${score}/10`, C.gold);
    ...
    typing_shooter: {
      ...
      category: "code",
      accent: C.cyan,
      instructions: "Type falling keywords & press ENTER — clear 10 before they hit bottom.",
      duration: 55000,
      create(w, h, keys, mouse) {
        ...
        const onKey = (e) => {
          if (e.key === "Enter") {
            ...
            if (match) { match.y = -99; typed = ""; cleared++; if (cleared >= 10) endSession("win"); }
            else typed = "";
        ...
        return {
          update() {
            frame++;
            if (frame % 55 === 0) falling.push({ word: words[rand(0, words.length - 1)], x: randF(40, w - 80), y: 35 });
            falling.forEach(f => { f.y += 1.8; if (f.y > h - 20) endSession("lose", "Unhandled exception!"); });
            falling = falling.filter(f => f.y < h);
          },
          draw(ctx) {
            ctx.fillStyle = C.bg;
            ctx.fillRect(0, 0, w, h);
            drawHud(ctx, w, `CLEARED: ${cleared}/10  TYPE: ${typed}`, C.cyan);
  ...
  function renderShell(arcadeId, extraStyle) {
    ...
    return `
      <div class="arcade-minigame" style="border-color:${meta.accent}; ${extraStyle || ""}">
        <div class="arcade-minigame-header">
          ...
          <span class="arcade-badge">ARCADE</span>
        </div>
        ...
        <p class="arcade-controls-hint">Keyboard: WASD / Arrows · Space · Click on canvas for focus</p>
        <div class="status-bar-track arcade-timer">
